 ...
 * implementation that ships now. When user approves a live rail for a
...
export interface CheckoutProvider {
   ...
   *  server-side class user reviews. */
  ...
  if (providerId !== TEST_PROVIDER_ID) {
    const err = new Error(
      `checkout-provider: live provider "${providerId}" is unavailable — ` +
        ...
        `A live rail needs user's approval and a server-side implementation.`
